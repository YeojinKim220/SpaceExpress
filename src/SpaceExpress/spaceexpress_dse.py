...
def spline_multi_rep(df, k):
    ...
    #   
    # } else {
        # running the full model and getting predicted_full  
        formula = as.formula(paste0("gene~ (",paste0("X",seq(1,df),collapse = " + "), ")* group + cell_type + ", "(1 | rep)"))
        full = lmer(formula = formula,data = data,control=lmerControl(calc.derivs = FALSE))
        ...
        # running the null model and getting predicted_null
        formula1 = as.formula(paste0("gene~ ",paste0("X",seq(1,df),collapse = " + "), " + group + cell_type ", "+ (1 | rep) "))
        null = lmer(formula = formula1,data = data)
