...
def spline_multi_rep(df, k):
    ...
    val = quantile(data$gene,0.99)
    
    rm_index = which(data$gene >= val)
    
    if (length(rm_index)!= 0) {
        data = data[-rm_index,]  # retaining the existing 99th-percentile filter
    }
...
def SpaceExpress_DSE(emb, adata_list, cell_type = None, k = 300, n_jobs=-1, multi = False, 
    ...
    Inputs:
    adata_list (list): AnnData objects with the same unique gene names in
        the same order. Mismatched genes are rejected before inputs are modified.
    n_jobs (int): The number of parallel jobs to run
    Expression preprocessing belongs in se.preprocessing(), before training.
    DSE does not apply mean+SD outlier removal. Multi-replicate fits retain
    their 99th-percentile filter and replicate-specific SD scaling.
    
    Returns:
    df_fdr (pd.DataFrame): A pandas DataFrame containing the false discovery rates
    """
    # DSE extracts expression columns by position, so validate names before
    # fitting or attaching embeddings to any of the caller's AnnData objects.
    if len(adata_list) < 2 or (not multi and len(adata_list) != 2):
        raise ValueError('DSE requires exactly two samples when multi=False, and at least two when multi=True.')
    reference_genes = adata_list[0].var_names
    if len(reference_genes) == 0:
        raise ValueError('DSE requires at least one gene.')
    for sample_index, adata in enumerate(adata_list):
        if not adata.var_names.is_unique:
            raise ValueError(f'Sample {sample_index} has duplicate gene names; DSE requires unique gene names.')
        if not adata.var_names.equals(reference_genes):
            raise ValueError(
                f'Sample {sample_index} must have the same genes in the same order as sample 0. '
                'Align var_names across all samples before calling SpaceExpress_DSE.'
            )
