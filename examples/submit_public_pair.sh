...
CPU_PARTITION=${SE_CPU_PARTITION:-cpu-medium}
GPU_CONSTRAINT=()
if [[ -n "${SE_GPU_CONSTRAINT:-}" ]]; then
  GPU_CONSTRAINT=(--constraint="$SE_GPU_CONSTRAINT")
fi
...
gpu=$(sbatch "${COMMON[@]}" "${GPU_CONSTRAINT[@]}" --job-name=se015-gpu --partition="${SE_GPU_PARTITION:-gpu-h100}" \
      --cpus-per-task=8 --mem=256G --gres=gpu:1 --dependency="afterok:$prep" \
